  ...
  return(
    <form className="flex flex-col px-[10%] md:px-0 md:mt-[10%] md:ml-[10%] mb-[26%]">
      <h1 className="font-bold text-2xl md:text-4xl text-secondary my-6 md:mt-0 md:mb-14 text-center">Login</h1>
        ...
        <input type="text" id="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="Enter your email address" className="input-auth mb-8" />
        <label htmlFor="password" className="font-bold text-xl mb-3">Password :</label>
        <input type="password" id="password" value={password} onChange={event => setPassword(event.target.value)} placeholder="Enter your password" className="input-auth mb-5" />
        <Link to="/forgotpwd" className="mb-14 font-bold text-xl text-secondary">Forgot Password?</Link>
        <button type="submit" className="btn h-14 md:h-[75px] text-xl text-secondary bg-primary rounded-2xl shadow-md shadow-primary mb-7" onClick={loginHandler}>Login</button>
        <button type="button" className="btn h-14 md:h-[75px] text-xl bg-white rounded-2xl shadow-2xl gap-3"><img src={logoGoogle} alt="" className="w-6 md:w-7" /> Login with Google</button>
    </form>
  )
}
...
function Register() {
  return(
    <form className="flex flex-col px-[10%] md:px-0 md:mt-[10%] md:ml-[10%] mb-[26%]">
      ...
      <label htmlFor="email" className="font-bold text-xl mb-3">Email Address :</label>
      <input type="text" id="email" placeholder="Enter your email address" className="input-auth mb-8" />
      <label htmlFor="password" className="font-bold text-xl mb-3">Email Address :</label>
      <input type="password" id="password" placeholder="Enter your password" className="input-auth mb-8" />
      <label htmlFor="phone" className="font-bold text-xl mb-3">Phone Number :</label>
      <input type="text" id="phone" placeholder="Enter your phone number" className="input-auth mb-8" />
...
function Auth() {
  ...
  const changeMode = () => {
    if(mode === "Login") return setMode("Sign Up");
    setMode("Login");
  };
  ...
  <section className="w-full flex flex-col md:flex-row">
    ...
    <div className="z-1 w-full max-w-[720px] flex flex-col pt-3 md:pt-14 md:mr-[10%] bg-white bg-opacity-70">
      <div className="flex px-[5%] md:pl-14">
        ...
        <button className="btn w-28 md:w-36 h-11 bg-primary text-secondary rounded-3xl shadow-2xl shadow-primary" onClick={changeMode}>{mode}</button>
      </div>
